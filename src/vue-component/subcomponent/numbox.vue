<template>
    <div>
        <div class="mui-numbox" data-numbox-min='1'>
					...
					<input id="test" class="mui-input-numbox" type="number" value="1" ref="inum" @change="numchange" />
					<button class="mui-btn mui-btn-numbox-plus" type="button">+</button>
		<!-- 传值给父组件【调用这个numbox的组件】，一是可以为这个加按钮和减按钮以及input绑定三个函数，来监听数值变化，
        简单的方法就是直接为这个text input 绑定 change事件，当input 输入框值改变就传值给父组件！！！ -->
        </div>
    </div>
</template>
